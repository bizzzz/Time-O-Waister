	...
	private void processSkeleton () {
		...
		for (int player = 0; player < 2; player++)
		{
			if (trackedPlayers[player] >= 0)
			{
				...
				if(player == 0 )
				{
					if(bonePos[0,11]. z - bonePos[0,1]. z > 0.25 && load)
					{
						GameObject g = GameObject.Find("Sphere");
						g.BroadcastMessage("Fire");
						load = false;
					}
					else if(bonePos[0,11]. z - bonePos[0,1]. z < -0.10 && !load )
					{
						load = true;
					}
					...
					if(Mathf.Abs(bonePos[0,7]. x - bonePos[0,8]. x) < 0.25)
					{
						if(Mathf.Abs(bonePos[0,7]. y - bonePos[0,4]. y )< 0.2)
							cube = GameObject.Find("Cube6");
						else if(bonePos[0,7]. y - bonePos[0,4]. y < -0.2)
							cube = GameObject.Find("Cube9");
						else
							cube = GameObject.Find("Cube3");
					}
					else if(Mathf.Abs(bonePos[0,7]. x - bonePos[0,4]. x) < 0.25)
					{
						if(Mathf.Abs( bonePos[0,7]. y - bonePos[0,4]. y )< 0.2 )
							cube = GameObject.Find("Cube5");
						else if(bonePos[0,7]. y - bonePos[0,4]. y < -0.2)
							cube = GameObject.Find("Cube8");
						else
						cube = GameObject.Find("Cube2");
					}
					else 
					{
						if(Mathf.Abs(bonePos[0,7]. y - bonePos[0,4]. y)< 0.2)
							cube = GameObject.Find("Cube4");
						else if(bonePos[0,7]. y - bonePos[0,4]. y < -0.2)
							cube = GameObject.Find("Cube7");
						else
						cube = GameObject.Find("Cube1");
					}
					
					light.BroadcastMessage("UpdateTarget", cube.transform);
					Sphere.BroadcastMessage("UpdateTarget", cube.transform);
				}
